...
class MochaJsonRunner extends Mocha.Runner {
  ...
  run() {
    ...
    function runSuite(suite) {
      ...
      suite.tests.forEach(function testFe(test) {
        switch (test.state) {
          case STATE_PASSED:
            ...
            if (!hasStats) self.stats.passes += 1;
            break;
          case STATE_FAILED:
            self.emit(EVENT_TEST_FAIL, test, test.err);
            if (!hasStats) self.stats.failures += 1;
            break;
          default:
            if (test.pending) {
              self.emit(EVENT_TEST_PENDING, test);
              if (!hasStats) self.stats.pending += 1;
            } else {
              throw new Error(
                `Unknown test.state: ${
                  test.state
        ...
        if (!hasStats) self.stats.tests += 1;
      });
      ...
      if (!hasStats) self.stats.suites += 1;
